...
from financelib.db_utils import insert_account_wallet_new, insert_wallet, select_wallet_by_iban
...
def create_wallet(payload, accessToken):
    ...
    wallet = select_wallet_by_iban(db_path, iban)
    print(wallet)

    # Wallet correctly inserted in the db
    if len(wallet) > 0:
        insert_account_wallet_new(db_path, account_id, wallet[0][0])
    ...
    # Wallet insertion error
        ...
        return {
            ...
            PAYLOAD: {
                ...
                'message': "An error occurred while creating new wallet."
            },
        }
