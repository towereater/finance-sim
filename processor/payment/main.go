...
func main() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Error while reading queue: %s", err.Error())
			continue
		}
		...
		if abi == "" || len(abi) != 5 {
			fmt.Printf("Queue content with key %s has invalid abi value", key)
			continue
		}
		...
		if err == mongo.ErrNoDocuments {
			fmt.Printf("No payment with id %s found", paymentId)
			continue
		}
		if err != nil {
