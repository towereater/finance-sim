...
public class BankApp
{
    ...
    public WalletManager WalletManager { get; private set; } = new WalletManager();
        ...
        while (true) {
            ...
            if (!AccountManager.IsLogged) {
                ...
                if (ans == 1) {
                    ...
                    string name = GetString("Insert your name (-q to exit): ");
                    if (pass == "-q")
                        continue;

                    string surname = GetString("Insert your surname (-q to exit): ");
                    if (pass == "-q")
                        continue;

                    string birth = GetString("Insert your birth date (YYYY-MM-DD format) (-q to exit): ");
                    if (pass == "-q")
                        continue;

                    string message = AccountManager.CreateAccount(user, pass, name, surname, birth);
                    Console.WriteLine($"{message}\n");
                }
            }
            else {
                ...
                if (ans == 1) {
                    string message = WalletManager.CreateWallet();
                    Console.WriteLine($"{message}\n");
                }
                else if (ans == 2) {
                    string message = WalletManager.GetWallets();
                    Console.WriteLine($"{message}\n");
                }
                else if (ans == 3) {
    ...
    private static int GetMenuOption(int maxValue)
    {
        int ans;
    ...
    private void PrintGUI()
    {
        if (!AccountManager.IsLogged) {
            ...
            Console.Clear();
            Console.WriteLine($"Welcome back, {AccountManager.UserAccount.Name}\n");
